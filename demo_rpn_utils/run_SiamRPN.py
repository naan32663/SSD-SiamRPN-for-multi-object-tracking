...
from demo_rpn_utils.utils import get_subwindow_tracking, get_subwindow
...
def tracker_eval(net, x_crop, target_pos, target_sz, window, scale_z, p):
    ...
    delta = delta.permute(1, 2, 3, 0).contiguous().view(4, -1).data.cpu().numpy() #permutepermute:将tensor的维度换位。contiguous: 使tensor变得连续
...
#    hc_z = target_sz[1] # + p.context_amount * sum(target_sz)
    ...
    z_crop = get_subwindow(im, target_pos, target_sz, p.exemplar_size, s_z, avg_chans, out_mode='np')
...
'''
# version 2
def SiamRPN_track(state, im, detec_pos, detec_sz):
    p = state['p']
    net = state['net']
...
#    x_crop = get_subwindow_tracking(im, target_pos, p.instance_size, round(s_x), avg_chans, out_mode='np')
    ...
    x_crop = get_subwindow(im, target_pos,target_sz, p.instance_size, round(s_z), avg_chans, out_mode='np')
#    print("Shape of x_crop = " + str(x_crop.shape))
#    plt.figure(figsize=(20,12))
#    plt.imshow(x_crop)
#    plt.axis('off')
    
#    print("x_crop shape = " + str(x_crop.shape))
    ...
    t_pos, t_sz, score = tracker_eval(net, x_crop.cuda(), target_pos, target_sz * scale_z, window, scale_z, p)
...
def SiamRPN_track(state, im):
    ...
    t_pos, t_sz, score = tracker_eval(net, x_crop.cuda(), target_pos, target_sz * scale_z, window, scale_z, p)
    target_pos[0] = max(0, min(state['im_w'], t_pos[0]))
    target_pos[1] = max(0, min(state['im_h'], t_pos[1]))
    target_sz[0] = max(10, min(state['im_w'], t_sz[0]))
    target_sz[1] = max(10, min(state['im_h'], t_sz[1]))
    state['target_pos'] = target_pos
    state['target_sz'] = target_sz
    state['score'] = score
    return state